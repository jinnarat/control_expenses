...
export class TravelingPage {
  ...
  addExpenses() {
    if (this.money == '' || this.description == '') {
      ...
      if (this.cd == 0) {
        ...
        this.userServie.addExpenses(this.result.userid, 'travel', this.description, this.money, this.date2, (docRef) => {
          if (this.sum < 0) {
            this.altClt.create({
          ...
          //     buttons: ['OK']
          //   }).present().then(() => {
          //     เม้นไว้อันหนึ่งเผื่อถามบุ๋ม   is.description = '';
          //     this.money = '';
          //   })
          // }
        })
  ...
  checkExpenses() {
    this.userServie.checkExpenses(this.result.userid, 'travel', this.date2, (income, expenses, arr, cd) => {
      this.income = income;
      ...
      buttons: [
        {
          ...
          handler: () => {
            ...
            this.userServie.deleteDetail(this.date2, 'travel', item.id, this.result.userid).then((doc) => {
              this.checkExpenses();
