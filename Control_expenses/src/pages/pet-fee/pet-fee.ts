...
@IonicPage()
@Component({
  selector: 'page-pet-fee',
  templateUrl: 'pet-fee.html',
})
export class PetFeePage {
  ...
  addExpenses() {
    if (this.money == '' || this.description == '') {
      ...
      if (this.cd == 0) {
        ...
        this.userServie.addExpenses(this.result.userid, 'pet', this.description, this.money, this.date2, (docRef) => {
          if (this.sum < 0) {
            this.altClt.create({
  ...
  checkExpenses() {
    ...
    this.userServie.checkExpenses(this.result.userid, 'pet', this.date2, (income, expenses, arr, cd) => {
      this.income = income;
      ...
      buttons: [
        {
          ...
          handler: () => {
            ...
            this.userServie.deleteDetail(this.date2, 'pet', item.id, this.result.userid).then((doc) => {
              this.checkExpenses();
